    ...
    <div className="register-progress__track">
      {stepLabels.map((label, i) => {
        ...
        return (
          <Fragment key={i}>
            {i > 0 && (
              <div className={['register-progress__line', completed && 'register-progress__line--done'].filter(Boolean).join(' ')} />
            )}
            <div className="register-progress__col">
              <div className={['register-progress__circle', completed && 'register-progress__circle--done', active && 'register-progress__circle--active'].filter(Boolean).join(' ')}>
                {completed ? <Check size={18} /> : idx}
              </div>
              <span className={['register-progress__label', active && 'register-progress__label--active'].filter(Boolean).join(' ')}>
                {label}
              </span>
            </div>
          </Fragment>
...
const PlanCard = ({ name, price, features, cta, highlighted, onClick }: {
  name: string; price: string; features: string[]; cta: string; highlighted?: boolean; onClick: () => void
}) => (
  <div className={['plan-card', highlighted && 'plan-card--highlighted'].filter(Boolean).join(' ')}>
    ...
    <button onClick={onClick} className={['plan-card__cta', highlighted ? 'plan-card__cta--primary' : 'plan-card__cta--outline'].join(' ')}>
      {cta}
    </button>
  </div>
)
  ...
  const next = async () => {
    ...
    if (step === 2) {
      ...
      try {
        const { error } = await supabase.auth.signUp({
          ...
          options: {
            emailRedirectTo: `${window.location.origin}/auth/register?verified=1`,
            data: {
    ...
    if (step === 3) {
      ...
      try {
        ...
        const { data: profile } = await supabase.from('st_profiles').select('*').eq('id', otpData.user!.id).single()
            ...
            <div key={step}>
                ...
                <div>
                  ...
                  <div className="auth-fields">
                    <div className="auth-field">
                      <input type="email" placeholder="empresa@mail.com" value={data.email} onChange={e => update('email', e.target.value)} className="auth-input" />
                    ...
                    <div className="auth-field">
                      <label className="auth-checkbox-label">
                        <input type="checkbox" checked={data.terms} onChange={e => update('terms', e.target.checked)} className="auth-checkbox" />
                        <span>
                  ...
                  <p className="auth-footer-text">
                    ¿Ya tenés cuenta?{' '}
                    <a href="#" onClick={e => { e.preventDefault(); router.push('/auth/login') }} className="auth-link">Iniciá sesión</a>
                  </p>
                </div>
              )}
              ...
              {step === 2 && (
                <div>
                  ...
                  <div className="auth-fields">
                    <div className="auth-field">
                      <input type="text" placeholder="Transportes S.A." value={data.companyName} onChange={e => update('companyName', e.target.value)} className="auth-input" />
                      {errors.companyName && <p className="auth-error">{errors.companyName}</p>}
                    </div>
                    <div className="auth-field">
                      <input type="text" placeholder="XX-XXXXXXXX-X" value={data.cuit} onChange={e => update('cuit', formatCuit(e.target.value))} className="auth-input" />
                      {errors.cuit && <p className="auth-error">{errors.cuit}</p>}
                    </div>
                    <div className="auth-field">
                      <input type="text" placeholder="Razón social oficial" value={data.legalName} onChange={e => update('legalName', e.target.value)} className="auth-input" />
                      {errors.legalName && <p className="auth-error">{errors.legalName}</p>}
                    </div>
                    <div className="auth-field">
                      <select value={data.industry} onChange={e => update('industry', e.target.value)} className="auth-input">
                    ...
                    <div className="auth-field">
                      <select value={data.fleetSize} onChange={e => update('fleetSize', e.target.value)} className="auth-input">
                    ...
                    <div className="auth-field">
                      <select value={data.country} onChange={e => update('country', e.target.value)} className="auth-input">
                    ...
                    <div className="auth-field">
                      <select value={data.province} onChange={e => update('province', e.target.value)} className="auth-input">
                  ...
                  <button onClick={next} className="auth-btn" disabled={loading}>
                ...
                <div>
                  ...
                  <div className="auth-code-inputs">
                    {data.code.map((c, i) => (
                      <input
                        ...
                        className={['auth-code-input', allCodeFilled ? 'auth-code-input--complete' : c ? 'auth-code-input--filled' : ''].filter(Boolean).join(' ')}
                      />
                    ))}
                  </div>
                  ...
                  <p className="auth-footer-text">
                    ...
                    {resendIn > 0 ? (
                      ...
                      <button onClick={handleResend} className="auth-link">Reenviar</button>
                    )}
                  </p>
                </div>
              )}
              ...
              {step === 4 && (
                <div>
                  ...
                  <p className="auth-subtitle" style={{ textAlign: 'center' }}>Comenzá a trackear tu flota hoy.</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <PlanCard name="Starter" price="$29" features={['Hasta 5 camiones', 'Tracking en tiempo real', 'App mobile para choferes', 'Historial 7 días', 'Soporte por email']} cta="Elegir Starter" onClick={() => choosePlan('starter')} />
                    <PlanCard name="Pro" price="$79" features={['Hasta 20 camiones', 'Todo lo de Starter', 'Historial 30 días', 'Alertas personalizadas', 'Panel multi-usuario (3 admins)', 'Soporte prioritario']} cta="Elegir Pro" highlighted onClick={() => choosePlan('pro')} />
                    <PlanCard name="Enterprise" price="$199" features={['Camiones ilimitados', 'Todo lo de Pro', 'Historial 1 año', 'API de integración', 'Reportes avanzados', 'Manager dedicado', 'SLA garantizado']} cta="Contactar ventas" onClick={() => choosePlan('enterprise')} />
                  </div>
                  <button onClick={() => setStep(3)} className="auth-back" style={{ margin: '1.5rem auto 0' }}>
                    <ArrowLeft size={16} /> Volver
                  </button>
                </div>
              )}
